...
class Radio_Buttons_for_Taxonomies {
	...
	*/
	...
	public function launch( $taxonomy ){
		...
		foreach($this->options['taxonomies'] as $single_post_and_taxonomy) {
			$actual_taxonomy = explode('-and-', $single_taxonomy)[1];
			$post_type = explode('-and-', $single_taxonomy)[0];
			if($actual_taxonomy == $taxonomy && $post_type == $this->get_current_post_type()) {
...
*/
	...
	public function block_editor_assets(){
		...
		// if any of this taxonomies match custom post type of usage now then it must be included in array 
		foreach($this->options['taxonomies'] as $single_post_and_taxonomy) {
			$actual_taxonomy = explode('-and-', $single_post_and_taxonomy)[1];
			$post_type = explode('-and-', $single_post_and_taxonomy)[0];

			if($post_type == $this->get_current_post_type()) {
				$taxonomy_array[] = $actual_taxonomy;

			}

		}

		// array must be given below
		wp_enqueue_script( 'radiotax-gutenberg-sidebar', plugins_url( 'js/dist/index.js', __FILE__ ), array( 'wp-i18n', 'wp-edit-post', 'wp-element', 'wp-editor', 'wp-components', 'wp-data', 'wp-plugins', 'wp-edit-post', 'wp-api' ), self::$version );
			...
			$i18n = array( 'radio_taxonomies' => (array) $taxonomy_array );
